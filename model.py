...
from utils import SyncBatchNorm
...
def get_hrnet_model(width=48, ocr_width=256, small=False, norm_radius=260,
                    ...
                    norm_layer=SyncBatchNorm, is_ritm=False):
...
class DistMapsHRNetModel(nn.Layer):
    def __init__(self,
        ...
        self.normalization = T.Normalize(mean=[.485, .456, .406], std=[.229, .224, .225])
    ...
    def forward(self, image, points):
        ...
        instance_out = nn.functional.interpolate(instance_out,
                                                 size=image.shape[2:],
                                                 mode='bilinear',
                                                 align_corners=True)
...
def get_deeplab_model(backbone='resnet18', deeplab_ch=256, aspp_dropout=0.5,
                      norm_layer=nn.BatchNorm2D, backbone_norm_layer=None,
                      use_rgb_conv=True, cpu_dist_maps=False,
                      norm_radius=260, is_ritm=False):
    model = DistMapsModel(
...
class DistMapsModel(nn.Layer):
    def __init__(self, feature_extractor, head, norm_layer=nn.BatchNorm2D, use_rgb_conv=True,
                 cpu_dist_maps=False, norm_radius=260, is_ritm=True):
    ...
    def forward(self, image, points):
        ...
        instance_out = nn.functional.interpolate(instance_out, size=image.shape[2:],
                                                 mode='bilinear', align_corners=True)
...
def get_shufflenet_model(norm_radius=260,
                         use_rgb_conv=True, cpu_dist_maps=True,
                         norm_layer=nn.BatchNorm2D, is_ritm=False):
